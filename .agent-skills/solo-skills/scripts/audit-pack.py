#!/usr/bin/env python3
"""Read-only audit and install-plan helper for bam-bam-2/solo-skills."""
from __future__ import annotations

import re

PACK_ID = "solo-skills"
EXPECTED_REMOTE = "https://github.com/bam-bam-2/solo-skills"
LICENSE_MARKER = "MIT License"
README_COUNT_MODE = "pattern"
README_COUNT_PATTERN = r"공개한 스킬\s+(\d+)개"
RISK_PATTERNS = {
    "archive_or_delete": re.compile(r"(?i)notion_archive|\"archived\"\s*:\s*true|즉시 아카이브"),
    "author_bound_paths": re.compile(r"~/Projects|getback|remote-host|mac-mini"),
    "credential_names": re.compile(r"(?i)(?:api[_ -]?key|token|password|secret)"),
    "external_send_or_publish": re.compile(r"(?i)threads_publish|SMTP_SSL|send_message|channels/.+/messages|발행 시작|메일 발송"),
    "long_numeric_ids": re.compile(r"(?<!\d)\d{15,20}(?!\d)"),
    "permission_bypass": re.compile(r"dangerously-(?:skip-permissions|bypass-approvals-and-sandbox)"),
    "remote_command": re.compile(r"(?m)^\s*(?:ssh|scp|rsync)\s|subprocess\.run"),
    "scheduler_mutation": re.compile(r"launchctl bootstrap|Library/LaunchAgents|\bcrontab\b"),
}
