...
function usage() {
  return [
    ...
    '',
    'Validates the local OpenOcta evaluation contract without calling a model.',
  ].join('\n');
}
...
function validate(payload) {
  ...
  if (payload.skill_name !== 'openocta') {
    fail('skill_name must equal "openocta"');
  }
  if (!Array.isArray(payload.evals) || payload.evals.length < 8) {
    fail('evals must contain at least eight cases');
  }
  ...
  for (let index = 0; index < payload.evals.length; index += 1) {
    ...
    const searchable = `${item.expected_output}\n${item.assertions.join('\n')}`;
    if (/confirmation|token|secret|read-only|refuses|blocks?|approval|without printing/i.test(searchable)) {
      safetyCaseCount += 1;
    }
    if (/routes? (away|to)|monitoring-observability|log-analysis|skill-standardization/i.test(searchable)) {
      routeOutCount += 1;
    }
  }

  if (safetyCaseCount < 5) {
    fail('eval suite must include at least five explicit safety-boundary cases');
  }
  if (routeOutCount < 1) {
    fail('eval suite must include at least one explicit route-out case');
  }
...
async function main() {
  ...
  try {
    ...
    if (jsonMode) {
      ...
      console.log(
        `PASS skill=${report.skill_name} evals=${report.evals} assertions=${report.assertions} safety_cases=${report.safety_cases} route_out_cases=${report.route_out_cases}`,
      );
    }
    return 0;
  } catch (error) {
